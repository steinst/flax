...
def filter_to_set(x: Filter) -> Set[str]:
  ...
  """
  assert not x and not isinstance(x, DenyList), 'Infinite set'
  if x is False:  # pylint: disable=g-bool-id-comparison
    return set()
  if isinstance(x, str):
    return set([x])
...
def union_filters(a: Filter, b: Filter) -> Filter:
  ...
  """
  if a is True or b is True:  # pylint: disable=g-bool-id-comparison
    return True
...
def subtract_filters(a: Filter, b: Filter) -> Filter:
  ...
  """
  if b is True:  # pylint: disable=g-bool-id-comparison
    return False
  if a is True:  # pylint: disable=g-bool-id-comparison
    return DenyList(b)
  if isinstance(a, DenyList) and isinstance(b, DenyList):
...
class Scope:
  ...
  def is_collection_empty(self, col: str) -> bool:
    ...
    if col in self.root._variables:  # pylint: disable=protected-access
      return not len(self.root._variables[col])   # pylint: disable=protected-access
    return True
...
def apply(fn: Callable[..., Any],
  ...
  def wrapper(variables: VariableDict,
    ...
    if mutable is not False:  # pylint: disable=g-bool-id-comparison
